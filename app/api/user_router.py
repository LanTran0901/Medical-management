...
from app.api.dependencies import (
    ...
    get_health_metric_readings_service,
...
@router.get(
    ...
    description=(
        ...
        "`health_metrics` (chỉ số đo theo thời gian). "
...
async def get_current_user_profile(
    ...
    health_metrics_svc: HealthMetricReadingsService = Depends(get_health_metric_readings_service),
) -> UserMeResponse:
    try:
        user = await GetUserUseCase(repository).execute(current_user.id)
    except ValueError as exc:
    ...
    metrics_by_profile = await health_metrics_svc.list_for_profiles(
        profile_ids, current_user.id, skip_access_check=True
    )
    ...
    for profile_ent in profile_entities:
        ...
        metric_items = metrics_by_profile.get(profile_ent.id, [])
